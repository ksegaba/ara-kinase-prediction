'''Calculate epistasis values using genotype estimated marginal means #

Arguments:
- dir_path: path to the directory containing the data files
- data_file: prefix of the data files

Outputs:
- _emmeans_epistasis.tsv: Epistasis values based on 4 established genetic
    interaction definitions (Mani 2008) using the estimated marginal means of
    each label. Each label is saved in a separate file in the same directory (dir_path).
- _medians_epistasis.tsv: Epistasis values based on 4 established genetic
    interaction definitions (Mani 2008) using the median genotype values of each
    label. Each label is saved in a separate file in the same directory (dir_path).
'''
# %%
import itertools
import os
...
def reshape_data(data, labels):
    ...
    tmp = data.groupby(
        'Set')['Genotype'].value_counts().reset_index('Genotype')
    print(sum(tmp.groupby('Set')['Genotype'].nunique()
          == 4) == tmp.index.nunique())  # True
    del tmp
    ...
    for trait in labels:
        for row in sorted_df.index:
            if sorted_df.loc[row, (trait, 'MA')] < sorted_df.loc[row, (trait, 'MB')]:
                ...
                sorted_df.loc[row, (trait, 'MA')
                              ] = sorted_df.loc[row, (trait, 'MB')]
...
def calc_relative_fitness(sorted_df):
    ...
    for trait in W_df.columns.levels[0]:
        W_df.loc[:, trait] = W_df[trait].apply(
            lambda x: x / x['WT'], axis=1).values
...
# data_file = 'fitness_data_for_Kenia_09232024'
dir_path = '/home/seguraab/ara-kinase-prediction/data/20250403_melissa_ara_data/corrected_data'
data_file = 'fitness_data_04032025'

for trait in ['PG', 'DTB', 'LN', 'DTF', 'SN', 'WO', 'FN', 'SPF', 'TSC', 'SH']:
    try:
        df = dt.fread(
            f'{dir_path}/{data_file}_{trait}_emmeans_with_no_int.tsv')
        df = df.to_pandas()
    except ValueError:
        print(f'File not found for {trait}. Skipping...')
        continue
    ...
    dfw = calc_epistasis(dfw,
                         f'{dir_path}/{data_file}_{trait}_emmeans_epistasis.tsv')
...
for trait in ['PG', 'DTB', 'LN', 'DTF', 'SN', 'WO', 'FN', 'SPF', 'TSC', 'SH']:
    ...
    # Calculate epistasis values
    dfw = calc_epistasis(
        dfw, f'{dir_path}/{data_file}_{trait}_medians_epistasis.tsv')
