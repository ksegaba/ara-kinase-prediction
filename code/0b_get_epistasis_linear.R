...
get_epi_stats <- function(set, label, set_df, formula, lmer=TRUE, itx=TRUE){
  ...
  result <- tryCatch({
    ...
    if (itx == FALSE){ # For models that exclude the interaction term
      e_est <- NA
      lowerCI <- NA
      upperCI <- NA
      rsquared <- NA
    ...
    if (itx == FALSE) {
      emm <- rename_with(emm, ~ paste0(label, '_no_int_', .))
      out_df <- left_join(set_df, emm, by=c('Genotype'=paste0(label, '_no_int_Genotype')))
      out_df <- out_df[, !(colnames(out_df) %in% c(paste0(label, '_no_int_MA'), paste0(label, '_no_int_MB')))]
    }
...
for (label in labels){
  ...
  if (label %in% list('SN', 'WO', 'FN', 'TSC')){
    ...
    join_cols <- c('Set', 'Flat', 'Column', 'Row', 'Number', 'Type', 'Genotype',
      'Subline', 'MA', 'MB', label, paste0(label, '_plog10'), paste0(label, '_plus1'),
      paste0(label, '_plus1_log10'))
    ...
    join_cols <- c('Set', 'Flat', 'Column', 'Row', 'Number', 'Type', 'Genotype',
      'Subline', 'MA', 'MB', label, paste0(label, '_plog10'), paste0(label, '_log10'))
  }
    ...
    if (new_label %in% colnames(data)){
      ...
      for (set in unique(data$Set)) {
        ...
        if (length(unique(set_df$Flat)) > 1) {
          ...
          form_no_int <- paste0(new_label, ' ~ MA + MB + (1|Flat)')
          lmer <- TRUE
        } else {
          formula <- paste0(new_label, ' ~ MA + MB + MA:MB')
          form_no_int <- paste0(new_label, ' ~ MA + MB')
          lmer <- FALSE
        }
        ...
        if (endsWith(new_label, '_log10')) { # deal with infinite values
          ...
          if (nrow(set_df_sub) == 0) {
            ...
            if (length(unique(set_df_sub$Genotype)) < 4) {
              next
            } else {
              out <- get_epi_stats(set, new_label, set_df_sub, formula, lmer)
              out_no_int <- get_epi_stats(set, new_label, set_df_sub, form_no_int, lmer, itx=F)
            }
          }
        } else {
          out <- get_epi_stats(set, new_label, set_df, formula, lmer)
          out_no_int <- get_epi_stats(set, new_label, set_df, form_no_int, lmer, itx=F)
        }
        ...
        if (is.null(out$model) == FALSE) {
          ...
          if (lmer == TRUE) { # Whether to run a mixed-effects model or not
            ...
            residuals2 <- resid(out_no_int$model) # For model without the interaction term
            fitted_vals2 <- fitted(out_no_int$model)
            sw_test2 <- shapiro.test(residuals2) # 1. Normality of residuals
            bp_test2 <- ncvMLM(out_no_int$model, bp=T) # 2. Homoscedasticity
            dw_test2 <- check_autocorrelation(out_no_int$model, nsim=10000)[[1]] # 3. Independence of residuals
            
            assumptions <- list(sw_test$statistic[[1]], sw_test$p.value, NA,
              bp_test, NA, dw_test, sw_test2$statistic[[1]], sw_test2$p.value,
               NA, bp_test2, NA, dw_test2)
            ...
            residuals2 <- resid(out_no_int$model)
            fitted_vals2 <- fitted(out_no_int$model)
            sw_test2 <- shapiro.test(residuals2)
            bp_test2 <- bptest(out_no_int$model)
            dw_test2 <- dwtest(out_no_int$model)

            assumptions <- list(
              sw_test$statistic[[1]], sw_test$p.value,
              bp_test$statistic[[1]], bp_test$p.value[[1]],
              dw_test$statistic[[1]], dw_test$p.value,
              sw_test2$statistic[[1]], sw_test2$p.value,
              bp_test2$statistic[[1]], bp_test2$p.value[[1]],
          ...
          # Collect the results
          if (counter2 == 1) emm_df <- rbind.fill(emm_df, left_join(out$out_df, out_no_int$out_df, by=join_cols))
          if (counter2 != 1) emm_df2 <- rbind.fill(emm_df2, left_join(out$out_df, out_no_int$out_df, by=join_cols))
          if (counter == 1) res <- rbind(res, c(out$results, assumptions, aov_res[2,ncol(aov_res)]))
          if (counter != 1) res2 <- rbind(res2, c(out$results, assumptions, aov_res[2,ncol(aov_res)]))
        }
        ...
        remove(set_df, out, out_no_int, assumptions, aov_res) # clear memory
      ...
      if (counter2 != 1) emm_df <- left_join(emm_df, emm_df2, by=join_cols, keep=F)
      counter2 <- counter2 + 1
  ...
  if (counter == 1) {
    colnames(res) <- c('Set', 'e_est', 'lowerCI', 'upperCI', 'rsquared',
      ...
      'bp_test_pval', 'dw_test_stat', 'dw_test_pval', 'no_int_sw_test_stat',
      'no_int_sw_test_pval', 'no_int_bp_test_stat', 'no_int_bp_test_pval',
      'no_int_dw_test_stat', 'no_int_dw_test_pval', 'int_term_pval')
  }
  if (counter != 1) {
