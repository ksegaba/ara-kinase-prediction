...
def calc_feature_values(gene1_values_df, gene2_values_df, cols, calc_type):
	...
	if calc_type in ['Number in pair', 'Pair total']:
		...
		result = np.full_like(gene1_values_df.values, None)  # Default to None for unsupported calc_type
	
	# Crear un DataFrame con los resultados
	result_df = pd.DataFrame(result, columns=[f"{col}_{calc_type.lower().replace(' ', '_')}" for col in cols])
...
def calc_continuous_feat_values(col_list, paml_feat, checklist, feat_set="paml"):
	...
	gene1_values_df = pd.DataFrame.from_records(gene1_values.values, index=genes.gene1)
	gene2_values = gene2_values.apply(lambda x: na_vals if pd.isna(x) else x)
	gene2_values_df = pd.DataFrame.from_records(gene2_values.values, index=genes.gene2)
	...
	cols_features_df = cols_features_df.loc[:, ~cols_features_df.columns.duplicated()]
...
if __name__ == '__main__':
	...
	features['Ka/Ks'] = calc_continuous_feat_values(ka_ks_cols, paml_feat, checklist)
	...
	for i, col in enumerate(col_list):
		...
		features[feat_name[i]] = calc_continuous_feat_values([col], feat1, checklist, feat_set='')
