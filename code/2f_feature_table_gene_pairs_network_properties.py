    ...
    if feature_type == 'continuous':
        ...
        for i in range(len(gene_pairs)):
            ...
            try:
                ...
                total = len(set(list(G.neighbors(gene1)) + list(G.neighbors(gene2))))
        ...
        col_names=['gene1', 'gene2', '# overlapping', 'Total #', '% overlapping']
            ...
            if col.split('_')[0] in subset['Calculation for gene pair'].unique():
                if col.endswith('_bin'):
                    new_col = subset.loc[(subset['NEW Feature name'] == feature_name) &
                        (subset['Calculation for gene pair'] == col.split('_')[0])
...
if (aranet_features.gene1.equals(co_expr_features.gene1)) &\
    ...
    feat_table.to_csv('data/Kinase_genes/features/TAIR10_features_network_properties.txt', sep='\t', index=False)
